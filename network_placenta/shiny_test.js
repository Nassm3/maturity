...
import('https://webr.r-wasm.org/v0.2.2/webr.mjs').then(async ({ WebR }) => {
  ...
  // Function for sending a message to webR and waiting for a response
  function sendMessageAndWaitForResponse(message) {
    // Return a new promise
    return new Promise((resolve, reject) => {
      // Event listener for messages from webR
      const messageListener = function(event) {
        // Check if the message received is a response to our message
        if (event.data.startsWith(message + ':')) {
          // Extract the response from the message received
          const response = event.data.substring(message.length + 1);

          // Resolve the promise with the response received
          resolve(response);

          // Remove the event listener
          webR.removeEventListener('message', messageListener);
        }
      };

      // Add the event listener
      webR.addEventListener('message', messageListener);

      // Send the message to webR
      webR.postMessage(message);
    });
  }

  // Example of using the sendMessageAndWaitForResponse function
  sendMessageAndWaitForResponse('Bonjour de la part de l\'application Netlify !').then(function(response) {
